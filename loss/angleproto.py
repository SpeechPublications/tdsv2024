...
class LossFunction(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        #print("We are in angleproto loss")
        #print(x.shape)
        out_anchor      = torch.mean(x[:,1:,:],1)
        out_positive    = x[:,0,:]
        #print(out_anchor.shape) #B,512
        #print(out_positive.shape) #B,512
        stepsize        = out_anchor.size()[0]
        #print(stepsize)
        #print(out_positive.unsqueeze(-1).shape)
        #print(out_anchor.unsqueeze(-1).transpose(0,2).shape)
